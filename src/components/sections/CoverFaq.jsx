...
import { coverFaq } from '../../data/coverblocks.js'

export default function CoverFaq() {
  const [activeTab, setActiveTab] = useState(0)
  const [openIndex, setOpenIndex] = useState(0)

  const questions = coverFaq.tabs[activeTab].questions
        ...
        <div className="max-w-3xl mx-auto flex flex-col gap-3">
          {questions.map((item, i) => {
            ...
            return (
              <div
                key={item.question}
                className="rounded-xl2 border border-gray-100 shadow-card overflow-hidden bg-white"
              >
                <button
                  type="button"
